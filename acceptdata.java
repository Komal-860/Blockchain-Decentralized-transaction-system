...
public class acceptdata extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		try 
		{
			String sqls = "SELECT * FROM acc_details WHERE uname ='" + email	+ "' and bank_id='"+id+"'";
			...
			if (rs.next()) 
			{
				...
				s.executeUpdate("update acc_details set accno='"+accNo+"' where uname ='" + email + "' and bank_id='"+id+"'");
				
				Statement ss=con.createStatement();
				ss.executeUpdate("update tblaccountbank set Accountno='"+accNo+"',Status='Active' where UserName ='" + email + "' and bank_id='"+id+"'");
				
				Statement sss=con.createStatement();
				sss.executeUpdate("UPDATE `tblaccountbank` SET `Status`='Accept' WHERE bid='"+id+"'");
			...
			else 
			{
				...
				if (rs1.next()) 
				{
					String sqlss = "INSERT INTO acc_details (accno, uname, acc_type, details, bank_id,Status ) VALUES ('"+ accNo+ "','"
						+ email
						+ "','"
